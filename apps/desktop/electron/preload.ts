...
const api = {
  // ── Downloads ──────────────────────────────────────────────────────────────
  download: {
    add:     (opts: AddDownloadOptions)        => ipcRenderer.invoke(IPC_CHANNELS.DOWNLOAD_ADD, opts),
    ...
    remove:  (id: string, del?: boolean)       => ipcRenderer.invoke(IPC_CHANNELS.DOWNLOAD_REMOVE, id, del),
    list:    ()                                => ipcRenderer.invoke(IPC_CHANNELS.DOWNLOAD_LIST),
    open:    (id: string)                      => ipcRenderer.invoke(IPC_CHANNELS.DOWNLOAD_OPEN, id),
    openDir: (id: string)                      => ipcRenderer.invoke(IPC_CHANNELS.DOWNLOAD_OPEN_DIR, id),
  },

  // ── Queue ──────────────────────────────────────────────────────────────────
  queue: {
    list:   ()                                       => ipcRenderer.invoke(IPC_CHANNELS.QUEUE_LIST),
    create: (name: string, opts?: object)            => ipcRenderer.invoke(IPC_CHANNELS.QUEUE_CREATE, name, opts),
    update: (id: string, updates: object)            => ipcRenderer.invoke(IPC_CHANNELS.QUEUE_UPDATE, id, updates),
    delete: (id: string)                             => ipcRenderer.invoke(IPC_CHANNELS.QUEUE_DELETE, id),
    start:  (id: string)                             => ipcRenderer.invoke(IPC_CHANNELS.QUEUE_START, id),
    stop:   (id: string)                             => ipcRenderer.invoke(IPC_CHANNELS.QUEUE_STOP, id),
    stats:  ()                                       => ipcRenderer.invoke(IPC_CHANNELS.QUEUE_STATS),
  },

  // ── Scheduler ─────────────────────────────────────────────────────────────
  scheduler: {
    list:   ()                                       => ipcRenderer.invoke(IPC_CHANNELS.SCHEDULER_LIST),
    create: (task: object)                           => ipcRenderer.invoke(IPC_CHANNELS.SCHEDULER_CREATE, task),
    update: (id: string, updates: object)            => ipcRenderer.invoke(IPC_CHANNELS.SCHEDULER_UPDATE, id, updates),
    delete: (id: string)                             => ipcRenderer.invoke(IPC_CHANNELS.SCHEDULER_DELETE, id),
    toggle: (id: string)                             => ipcRenderer.invoke(IPC_CHANNELS.SCHEDULER_TOGGLE, id),
  },
